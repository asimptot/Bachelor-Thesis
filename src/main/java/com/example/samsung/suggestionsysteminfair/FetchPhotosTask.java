...
public class FetchPhotosTask extends AsyncTask<String, Void, String[]> {

    private final String LOG_TAG = FetchPhotosTask.class.getSimpleName();
    private Activity activity;

    public FetchPhotosTask(Activity activity) {
    ...
    private String[] getNextVenuesDataFromJson(String forecastJsonStr)
        ...
        final String OWM_PREFIX = "prefix";
        final String OWM_SUFFIX = "suffix";

        JSONObject responseJson = new JSONObject(forecastJsonStr);
        JSONObject nextVenuesObj = responseJson.getJSONObject(OWM_RESPONSE).getJSONObject(OWM_PHOTOS);

        JSONArray itemsList = nextVenuesObj.getJSONArray(OWM_ITEMS);

        String[] resultStrs = new String[itemsList.length()];
        for (int i = 0; i < (itemsList.length() < 5 ? itemsList.length() : 5); i++) {
            resultStrs[i] = itemsList.getJSONObject(i).getString(OWM_PREFIX) + "500x500" +
                    itemsList.getJSONObject(i).getString(OWM_SUFFIX);
            Log.i(LOG_TAG, "Next photo entry: " + resultStrs[i]);
        }
        return resultStrs;
    ...
    protected String[] doInBackground(String... params) {
        ...
        try {
            ...
            final String FORECAST_BASE_URL =
                    "https://api.foursquare.com/v2/venues/" + params[0] + "/photos";
    ...
    protected void onPostExecute(String[] result) {
        if (result != null && result.length >= 1) {
            Intent intent = new Intent(activity, VenuePhotosActivity.class);
            Bundle bundle = new Bundle();
            bundle.putStringArrayList("photosList", new ArrayList<>(Arrays.asList(result)));
            intent.putExtras(bundle);
            activity.startActivity(intent);
        } else {
            Toast.makeText(activity, "No photos", Toast.LENGTH_SHORT).show();
        }
    }
}
